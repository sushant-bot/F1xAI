...
from app.config import CACHE_DIR, FASTF1_CACHE_ENABLED, PRIMARY_RACE, DEBUG_MODE
...
def load_race(
    ...
    try:
        ...
        race_lap_count = 0
        if not session.laps.empty and "LapNumber" in session.laps.columns:
            lap_numbers = pd.to_numeric(session.laps["LapNumber"], errors="coerce").dropna()
            race_lap_count = int(lap_numbers.max()) if not lap_numbers.empty else 0

        stats = {
            "year": year,
            "event": event,
            "session_type": session_type,
            "total_laps": race_lap_count,
...
def get_pit_strategies(session: Session) -> pd.DataFrame:
    ...
    # Build a DriverNumber -> Abbreviation map so API always uses driver codes like VER/HAM.
    driver_code_map = {}
    try:
        results = session.results[["DriverNumber", "Abbreviation"]].copy()
        for _, row in results.iterrows():
            driver_num = str(row.get("DriverNumber", ""))
            driver_code = str(row.get("Abbreviation", "")).strip()
            if driver_num and driver_code:
                driver_code_map[driver_num] = driver_code
    except Exception:
        driver_code_map = {}
    ...
    for driver in session.drivers:
        ...
        for stint in driver_laps["Stint"].unique():
            ...
            strategy_data.append({
                "Driver": driver_code_map.get(str(driver), str(driver)),
                "Stint": int(stint),
...
def get_race_results(session: Session) -> pd.DataFrame:
    ...
    results = session.results[base_columns].copy()
